...
<body class="bg-gray-100" data-page="dosen">
    <div class="flex">
        ...
        <main class="flex-1 p-6">
            <h2 class="text-center text-4xl font-bold">.::Data Mata Kuliah::.</h2>
            <div class="bg-white shadow-md p-4 rounded-lg mt-4">
                <div class="flex justify-between mb-4">
                    <a href="{{route('Matkul.create')}}" class="bg-blue-500 text-white px-4 py-2 rounded">+ Tambah Data</a>
                    <input type="text" id="searchInput" placeholder="Cari Matkul..." class="border p-2 rounded w-1/3">
                </div>
                <table class="w-full mt-4 border-collapse border border-gray-300">
                    ...
                    <tbody id="matkulTable">
                        ...
                        <tr>
                            ...
                            <td class="border p-2">{{ $m['kode_matkul'] }}</td>
                            <td class="border p-2">{{ $m['nama_matkul'] }}</td>
                            <td class="border p-2">{{ $m['sks'] }}</td>

                            <td class="border p-2 text-center flex gap-2 justify-center">

                                <a href="{{ route('Matkul.edit', $m['kode_matkul']) }}" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600">Edit</a>

                                <form action="{{ route('Matkul.destroy', $m['kode_matkul']) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
